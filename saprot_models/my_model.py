...
def dssp_labels(pdb_id, pdb_path):
    numeric_label = []
    annotated = pd.read_csv(os.path.join(os.getcwd(),"cc.csv"),header=0, sep="\t")
...
print(classification_report(labels_seq, preds_seq,labels=list(idx_to_class.keys()), target_names=list(class_to_idx.keys())))

print("\nSequence+3Di classification report:")
print(classification_report(labels_struct, preds_struct,labels=list(idx_to_class.keys()), target_names=list(class_to_idx.keys())))
