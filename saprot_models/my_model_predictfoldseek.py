...
for pdb in pdbs.index:
    ...
    if len(foldseek_seq) != seq_emb.shape[0]:
        raise ValueError(f"\tLength mismatch: emb_seq {seq_emb.shape[0]}, labels {len(foldseek_seq)}")
    elif struct_aware_emb.shape[0] != len(foldseek_seq):
        raise ValueError(
            f"\tLength mismatch: emb_struct {struct_aware_emb.shape[0]}, labels {len(foldseek_seq)}")
    else:
        print(f"\t\u2713 Equal embeddings and DSSP labels shape ({struct_aware_emb.shape}, {len(foldseek_seq)}).")
...
print(classification_report(labels_seq, preds_seq,labels=class_to_idx.values(), target_names=class_to_idx.keys()))

print("\nSequence+3Di classification report:")
print(classification_report(labels_struct, preds_struct,labels=class_to_idx.values(), target_names=class_to_idx.keys()))
