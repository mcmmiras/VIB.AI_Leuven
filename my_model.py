...
def foldseek_labels(pdb_path, selected_chain, labelnum, labels_dssp):
...
def train_test_subsets(emb, labels_np):
    ...
    # Training balance for equal class distribution
    y_train = labels_np[train_idx]
    class0_idx_train = train_idx[y_train == 0]
    class1_idx_train = train_idx[y_train == 1]

    n = min(len(class0_idx_train), len(class1_idx_train))

    np.random.seed(42)
    class0_bal_train = np.random.choice(class0_idx_train, n, replace=False)
    class1_bal_train = np.random.choice(class1_idx_train, n, replace=False)

    train_idx = np.concatenate([class0_bal_train, class1_bal_train])
    y_train = labels_np[train_idx]
...
train_seq_emb, test_seq_emb, y_train, y_test = train_test_subsets(all_seq_embs, all_labels_num)
...
train_struct_aware_emb, test_struct_aware_emb, y_train, y_test = train_test_subsets(all_struct_aware_embs, all_labels_num)
